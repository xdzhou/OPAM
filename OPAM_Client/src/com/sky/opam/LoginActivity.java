...
public class LoginActivity extends Activity {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        monBtn.setOnClickListener(new android.view.View.OnClickListener() {			
			public void onClick(View v) {
				...
				if(login.length()==0) showInfo("input your login, please!");
				else if(password.length()==0) showInfo("input your password, please!");
				else {				        
			        u = worker.findUser(login);
			        
			        if(u==null){downloadCharge();}
			        else{
	...
	private void askForUpdate(){		
        new AlertDialog.Builder(this)
        .setTitle("update").setMessage("Your courses are a bit outdated, do you want to update ?")
    ...
    private void AgendaShow(){
    	...
    	if(!app.getUserID().equals("") && !app.getUserID().equals(login+"*"+u.getUsename())){
    		Intent service = new Intent(this, WebViewService.class);
			stopService(service);
    	}
    	app.setUserID(login+"*"+u.getUsename());
    	app.setUserName(u.getUsename());
    	
        Intent intent = new Intent();
        intent.setClass(LoginActivity.this, AgendaActivity.class);
	...
	private void ClearBeforeFinishApp(){
		Intent broadCast = new Intent();
		broadCast.setAction("com.sky.clientSide");
		TranObject tranObject = new TranObject();
		tranObject.setType(TranObjectType.ServiceStop);
		broadCast.putExtra("msg", tranObject);
		sendBroadcast(broadCast);
		System.out.println("require service Stop!");
	}
	...
	class downloadTask extends AsyncTask<String, Integer, String>{
		...
		protected String doInBackground(String... arg0) {			
			...
			if(resulta.equals("OK")){			
				try {
					DataCompo dataCompo = PullXMLReader.readXML(new ByteArrayInputStream(rspXML.getBytes("UTF-8")));
